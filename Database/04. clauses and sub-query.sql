...
INSERT INTO product (product_name, category, price, stock, supplier, release_date) VALUES
...
select count(*) from product where release_date > '2023-03-01' ;

select * FROM product WHERE price > 500  and stock > 5 ;

SELECT * FROM product WHERE stock between 10 and 20 ;

SELECT * FROM product WHERE product_name in ("laptop", "cookware set", "tablet") ;
...
SELECT supplier, avg(stock) AS total_stock FROM product GROUP BY supplier;

SELECT YEAR(release_date) AS release_year, COUNT(*) AS product_count FROM product 
GROUP BY release_year;

SELECT month(release_date) AS release_year, COUNT(*) AS product_count FROM product 
GROUP BY release_year;
...
select supplier, avg(price) as average_price from product group by supplier
...
SELECT * FROM product ORDER BY price DESC, stock asc;

SELECT category, stock FROM product ORDER BY category DESC, stock asc;
...
SELECT * FROM product LIMIT 11 offset 19;

---- distinct 

select distinct category from product ;

select unique category from product ;

SELECT category, AVG(price) AS average_price, COUNT(*) AS product_count FROM product
...
SELECT category, COUNT(*) AS product_count FROM product WHERE category IN 
( SELECT category FROM product GROUP BY category HAVING SUM(stock) > 50 )
GROUP BY category;

-- List Suppliers with More Products than the Average

SELECT supplier FROM product GROUP BY supplier HAVING COUNT(*) > 
(SELECT AVG(product_count) FROM (SELECT COUNT(*) AS product_count FROM product GROUP BY supplier)
 AS avg_count );
